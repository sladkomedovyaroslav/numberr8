/**
 * main.js - Основной JavaScript для ресторана «Вкус Востока»
 * Включает: AJAX-отправку формы, валидацию, мобильное меню, плавную прокрутку
 */

document.addEventListener('DOMContentLoaded', () => {
    'use strict';
    
    // Проверяем, включен ли JavaScript (для фоллбека)
...
function initBookingForm() {
    ...
    function showErrors(errors) {
        ...
        for (const [field, message] of Object.entries(errors)) {
            ...
            let input;
            if (field === 'languages') {
                input = form.querySelector('#languages');
    ...
    function clientValidate() {
        ...
        // Правила валидации (синхронизированы с сервером)
        if (!fullName) {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        // Без .htaccess используем api.php с GET-параметром для PUT
        let url = 'api.php';
        let method = 'POST';
    ...
    async function loadUserData() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Меняем текст кнопки для авторизованных
                if (submitBtn) {
